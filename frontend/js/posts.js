...
async function loadPosts() {
  try {
    const url = apiUrl(
      `/api/posts?page=${currentPage}&size=${pageSize}&sortBy=${sortBy}&sortDir=${sortDir}`,
    );
...
async function deletePost(id) {
  if (!confirm("Are you sure you want to delete this post?")) return;
  try {
    await apiDelete(apiUrl(`/api/posts/${id}`));
    showToast("✅ Post deleted successfully!", "success");
    loadPosts();
  } catch (err) {
    console.error("Delete error:", err);
...
// Uses shared API helpers from main.js (apiGet/apiDelete)
